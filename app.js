...
// Mídia só vê Início, Membros, Repertório e Perfil
...
const scaleChannel = supabaseClient.channel('scale-changes')
...
const scaleItemsChannel = supabaseClient.channel('scale-items-changes')
...
const scaleSongsChannel = supabaseClient.channel('scale-songs-changes')
...
const keyChannel = supabaseClient.channel('key-changes')
...
const dailyMsgChannel = supabaseClient.channel('daily-message-changes')
...
// ==========================================
// MEMBROS - BUG CORRIGIDO (query resiliente)
// ==========================================
...
if(!container) return;
try {
// Garantir que a Pasta Geral exista
await ensureGeneralFolder();
const res = await fetch(`${SUPABASE_URL}/rest/v1/folders?select=*&order=is_general.desc,name.asc`, { headers });
const folders = await res.json();
allFoldersCache = folders;
// Buscar nomes dos criadores
const creatorIds = [...new Set(folders.filter(f => !f.is_general).map(f => f.created_by).filter(Boolean))];
let creatorsMap = {};
if (creatorIds.length > 0) {
const creatorsRes = await fetch(`${SUPABASE_URL}/rest/v1/members?select=id,full_name&id=in.(${creatorIds.join(',')})`, { headers });
const creators = await creatorsRes.json();
creators.forEach(c => { creatorsMap[c.id] = c.full_name; });
}
let html = '';
folders.forEach(folder => {
const isActive = currentFolderId === folder.id || (currentFolderId === null && folder.is_general);
const canEdit = folder.is_general ? currentUserData.is_leader :
(folder.created_by === currentUserData.id || currentUserData.is_leader);
const creatorName = folder.is_general ? 'Sistema' : (creatorsMap[folder.created_by] || 'Desconhecido');
const subtitle = folder.is_general ? 'Todas as músicas' : `Por: ${creatorName}`;
html += `
<div class="folder-wrapper">
<button class="folder-item ${isActive ? 'active' : ''}" onclick="selectFolder('${folder.id}')">
<span class="material-symbols-outlined">${folder.is_general ? 'folder_special' : 'folder'}</span>
<div class="folder-info">
<span>${folder.name}</span>
<small>${subtitle}</small>
</div>
</button>
${canEdit && !folder.is_general ? `
...
container.innerHTML = html;
// Se nenhuma pasta selecionada, seleciona a Geral
if (!currentFolderId) {
const generalFolder = folders.find(f => f.is_general);
if (generalFolder) currentFolderId = generalFolder.id;
}
} catch (e) {
...
async function ensureGeneralFolder() {
try {
const res = await fetch(`${SUPABASE_URL}/rest/v1/folders?is_general=eq.true&select=id`, { headers });
const data = await res.json();
if (data.length === 0) {
await fetch(`${SUPABASE_URL}/rest/v1/folders`, {
method: 'POST', headers,
body: JSON.stringify({ name: 'Pasta Geral', is_general: true, created_by: currentUserData.id })
});
}
} catch(e) { console.warn('Erro ao garantir pasta geral:', e); }
}
...
await fetch(`${SUPABASE_URL}/rest/v1/folders`, {
method: 'POST', headers,
body: JSON.stringify({ name: name, created_by: currentUserData.id, is_general: false })
});
loadFolders();
...
try {
const generalFolder = allFoldersCache.find(f => f.is_general);
if (generalFolder) {
await fetch(`${SUPABASE_URL}/rest/v1/repertoire?folder_id=eq.${folderId}`, {
method: 'PATCH', headers,
body: JSON.stringify({ folder_id: generalFolder.id })
});
}
...
// ==========================================
// BUSCA ULTRA-PODEROSA DE MÚSICAS (MELHORADA)
// ==========================================
...
const lyricSelectors = ['article .cnt-ser', '.lyrics-container', '.lyrics', '[data-lyrics-container]', '.letra', '#letra', 'article'];
...
} catch(e) { console.warn('Erro ao extrair do letras.mus.br:', e); }
return null;
}
...
seen.add(key);
let lyrics = '';
if(url) lyrics = await extractLyricsFromLetras(url);
if(lyrics && lyrics.length > 50) {
foundAnyValid = true;
const uniqueId = `letras_${item.id || Math.random()}`;
cachedLyricsSearch[uniqueId] = { artist, song, lyrics, source: 'Letras.mus.br' };
...
} catch(e) { console.warn('YouTube falhou:', e); }
}
...
try {
// Se não há pasta selecionada, usar Pasta Geral
let folderId = currentFolderId;
if (!folderId) {
const generalFolder = allFoldersCache.find(f => f.is_general);
if (generalFolder) folderId = generalFolder.id;
}
...
vocalist: vocalist || null, folder_id: folderId || null
})
});
...
try {
let query = `${SUPABASE_URL}/rest/v1/repertoire?select=*,repertoire_keys(ton)&order=title.asc`;
if(currentFolderId) {
...
// Buscar criadores
const creatorIds = [...new Set(allRepertoireCache.map(s => s.created_by).filter(Boolean))];
let creatorsMap = {};
if (creatorIds.length > 0) {
const creatorsRes = await fetch(`${SUPABASE_URL}/rest/v1/members?select=id,full_name&id=in.(${creatorIds.join(',')})`, { headers });
const creators = await creatorsRes.json();
...
const ownerName = creatorsMap[song.created_by] || '';
const ownerInfo = ownerName ? `<small style="color:var(--text-muted); font-size:0.75rem;">Por: ${ownerName}</small>` : '';
// Permissão: dono da música, dono da pasta, ou líder
const currentFolder = allFoldersCache.find(f => f.id === currentFolderId);
const canEdit = song.created_by === currentUserData.id ||
(currentFolder && currentFolder.created_by === currentUserData.id) ||
...
/^(intro|introdução)\s*\d*$/i, /^(verso|verse)\s*\d*$/i,
/^(pr[eé]-?refr[aã]o|pre-?chorus)\s*\d*$/i, /^(refr[aã]o|chorus)\s*\d*$/i,
/^(ponte|bridge)\s*\d*$/i, /^(final|outro|encerramento)\s*\d*$/i,
/^(interlúdio|interlude)\s*\d*$/i, /^(solo)\s*\d*$/i
];
...
return blocks.map((block, idx) => ({ label: `Parte ${idx + 1}`, content: block.trim() }));
}
return verses;
}
...
if (item.sections.length === 0) { showCustomAlert(`"${item.songTitle}" não tem partes.`); return; }
}
try {
let folderId = currentFolderId;
if (!folderId) {
...
body: JSON.stringify({
title: title, is_medley: true, created_by: currentUserData.id,
lyrics_text: generateMedleyLyrics(), folder_id: folderId || null
})
});
...
method: 'POST', headers,
body: JSON.stringify({
medley_repertoire_id: medleyId, song_repertoire_id: item.songId,
section: section.label, section_content: section.content
})
});
}
}
...
else showCustomAlert('Erro ao atualizar voz/cantor.');
} catch(e) { showCustomAlert('Erro de conexão ao salvar.'); }
}
...
container.innerHTML = html;
} catch(e) { console.error('Erro ao carregar tons:', e); }
}
...
// ==========================================
// ESCALAS - CORRIGIDO (mostra função específica)
// ==========================================
...
try {
const res = await fetch(`${SUPABASE_URL}/rest/v1/scales?select=*,scale_items(role,members(id,full_name,photo_url,is_leader)),scale_songs(repertoire(title,repertoire_keys(ton),vocalist))&order=event_date.asc`, { headers });
...
// Mostrar função específica (role da escala) independente de ser líder
...
const icon = getRoleIcon(item.role);
const roleName = getRoleName(item.role);
const isLeader = item.members.is_leader;
const photoUrl = item.members.photo_url;
const leaderBadge = isLeader && item.role !== 'lider' ? '<span class="leader-mini-badge">★</span>' : '';
teamHtml += `
...
<span class="scale-team-name">${item.members.full_name} ${leaderBadge}</span>
...
memberId: i.member_id, role: i.role, name: i.members.full_name
}));
renderScaleDraftTeam();
if(allMembersCache.length === 0) {
const resMem = await fetch(`${SUPABASE_URL}/rest/v1/members?select=id,full_name,role`, { headers });
...
container.innerHTML = `
<div class="stat-card">
<div class="stat-icon" style="background: linear-gradient(135deg, #f05a28, #d94d1f);">
...
method: 'POST', headers: { ...headers, 'Prefer': 'return=representation' },
body: JSON.stringify({
username, full_name: fullname, email: email || null, phone: phone || null,
is_leader: isLeader, role: role
})
});
if (!res.ok) throw new Error('Usuário já existe ou erro ao cadastrar.');
const saved = await res.json();
// Criar pasta pessoal para o novo membro
await fetch(`${SUPABASE_URL}/rest/v1/folders`, {
method: 'POST', headers,
...
<div class="photo-placeholder" style="display:none">${m.full_name.charAt(0)}</div>` :
...
const data = await res.json();
if(data.length === 0) { showCustomAlert('Membro não encontrado.'); return; }
const m = data[0];
...
document.getElementById('edit-role').value = m.role || 'vocal';
document.getElementById('edit-is-leader').checked = m.is_leader || false;
document.getElementById('modal-edit-member').classList.add('active');
} catch(e) {
showCustomAlert('Erro ao carregar membro.');
}
}
...
is_leader: document.getElementById('edit-is-leader').checked
};
if(!updateData.full_name) { showCustomAlert('Nome é obrigatório.'); return; }
try {
const res = await fetch(`${SUPABASE_URL}/rest/v1/members?id=eq.${id}`, {
method: 'PATCH', headers, body: JSON.stringify(updateData)
});
if(!res.ok) throw new Error('Erro ao atualizar');
showCustomAlert('Membro atualizado!', 'Sucesso');
closeModals();
loadAdminMembers();
} catch(e) {
showCustomAlert('Erro ao atualizar membro.');
}
}
